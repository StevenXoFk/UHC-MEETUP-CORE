...
export class ActionForm {
   ...
   * @param {string} iconPath The icon path of the button. This is not required to add a button
  ...
  /**
   * @function send The sends the UI to a player
   * @param {Player} player The player you want to send it to
   * @param callback a asynchronous arrow function that will run after the form is filled out (Not fully if a model form).
   * @example .send(Player);
   * @returns {void}
   */
  send(player, callback = null) {
    this.form
      .show(player)
      .then((res) => {
...
export class ModalForm {
   ...
   * @returns {void}
   */
  addDropdown(label, options, defaultValueIndex = null) {
  ...
  addSlider(label, minimumValue, maximumValue, valueStep, defaultValue) {
    if (minimumValue >= maximumValue)
      throw new Error(
        "[Forms UI Silder] Error - the Min value cannot be greater than the Max value"
      );
    this.form.slider(
      label,
      minimumValue,
      maximumValue,
      valueStep ?? 1,
      defaultValue ?? ~~(maximumValue / minimumValue)
    );
  }
  /**
