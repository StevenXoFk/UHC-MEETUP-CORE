...
new SA.Command({
  name: "hoster",
  description: "dar rango host a alguien",
  tags: ["owner"],
})
  .addOption("player", "player", "Player to ban")
  .executes((ctx, { player }) => {
    SA.Providers.chat.broadcast(` §7Haz recibido el rango §dHost!`, player.nameTag );
...
.addSubCommand({
  ...
  .addOption("player", "player", "Player to ban")
  .executes((ctx, { player }) => {
    SA.Providers.chat.broadcast(` §7Se te ha removido el rango §dHost!`, player.nameTag );
